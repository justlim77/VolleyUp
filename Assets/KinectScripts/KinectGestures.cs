...
//							}
            ...
            case Gestures.HandsTogether:
                ...
                float distHands = vectorHands.sqrMagnitude;

                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
                           (Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[rightHandIndex].x) > 0.2f ||
                           Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[rightHandIndex].z) > 0.2f))
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                            gestureData.tagFloat = distHands;
                            gestureData.progress = 0.3f;
                        }
                        break;
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 0.5f)
                        {
                            ...
                            if (isInPose)
                            {
                                ...
                                gestureData.timestamp = timestamp;
                                gestureData.tagFloat = distHands;
                                gestureData.screenPos.z = distHands;
                                gestureData.progress = 0.7f;
                            }
                            else
                            {
                ...
                break;
            // check for HandsApart
            case Gestures.HandsApart:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] &&
                           Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[rightHandIndex].x) < 0.2f &&
                           Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[rightHandIndex].z) < 0.3f)
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
                            gestureData.progress = 0.3f;
                        }
                ...
                break;
            // check for UnderhandLeftToss
            case Gestures.UnderhandLeftToss:
                switch (gestureData.state)
                {
                    case 0:  // gesture detection - phase 1
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftHipIndex] && jointsTracked[leftElbowIndex] &&
                           jointsPos[leftHandIndex].y - jointsPos[leftHipIndex].y < 0f)
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftElbowIndex, jointsPos[leftElbowIndex]);
                            gestureData.progress = 0.1f;
                        }
                        break;
                    ...
                    case 1:  // gesture phase 2 = complete
                        if ((timestamp - gestureData.timestamp) < 1.5f)
                        {
                            bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[leftHipIndex] && jointsTracked[leftElbowIndex] &&
